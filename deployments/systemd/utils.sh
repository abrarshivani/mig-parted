...
# Seconds to wait for any single container runtime call before giving up.
cmd_timeout="${cmd_timeout:-30}"

# Reports whether a container runtime can be queried from a hook without
# blocking. A client call connects to the runtime's socket, and systemd
# socket-activates the unit behind it if it is not already running. On DGX
# hosts docker.service is ordered after nvidia-gpu-reset.target, which is
# ordered after nvidia-mig-manager.service, creating a deadlock. A runtime
# whose service is not already active has no running containers to kill, so
