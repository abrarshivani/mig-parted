...
# Resolves every module this repository builds from to its upstream repository and
...
# The bundled binary's dependencies are not vendored, so modules.txt alone does
# not name them. Its module list is read from the tools module instead.
...
fetch_retry() {
    ...
    body_tmp_file="$(mktemp "${TMPDIR:-/tmp}/mig-parted-fetch.XXXXXX")"
...
# Both module sets, in modules.txt's '# <path> <version>' shape. Keyed by module
# and not by version downstream, so a duplicate across the two is harmless.
module_lines() {
...
main() {
    ...
    [[ -d "${TOOLS_DIR}" ]] \
        || die "${TOOLS_DIR} not found — run 'make third-party-notices-repos' from the repo root."

    local repos_tmp_file unresolved=0
    repos_tmp_file="$(mktemp "${TMPDIR:-/tmp}/mig-parted-repos.XXXXXX")"
    ...
    # One row per module, not per module@version: the map is keyed by module,
    # and the two lists overlap at different versions of the same dependency.
    done < <(module_lines | awk '{print $2, $3}' | LC_ALL=C sort -u | awk '!seen[$1]++')
...

    {
        ...
        LC_ALL=C sort -u "${repos_tmp_file}"
    } > "${OUTPUT}"
